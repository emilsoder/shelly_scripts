...
function getLightStatus(callback) {
    Shelly.call("Light.GetStatus", { id: WHITE_CHANNEL_ID }, function (res, error_code, error_message) {
        if (error_code !== 0 || !res) {
...
Shelly.addEventHandler(function (event) {
    ...
    getLightStatus(function (currentBrightness, isOn) {
        ...
        for (let i = 0; i < pressCount; i++) {
            if (button === "btn_right_up") {
                newBrightness = Math.min(newBrightness + STEP, 100);
            } else if (button === "btn_right_down") {
                newBrightness = Math.max(newBrightness - STEP, 0);
            }
        }

        // Apply brightness after all presses are processed
      if (button === "btn_right_up" && action === "long_press") {
            turnOn();
        } else if (button === "btn_right_down" && action === "long_press") {
            turnOff();
        } else  if (button === "btn_right_up" || button === "btn_right_down") {
...
Shelly.addEventHandler(function (event) {
  ...
  if (
    !result ||
    (result.button !== "btn_up_right" && result.button !== "btn_down_right")
  ) {
    print("No valid button press detected.");
    return; // Avoid error by ensuring result is not null
  }
  ...
  getLightStatus(function (currentBrightness, isOn) {
    ...
    for (let i = 0; i < pressCount; i++) {
      if (button === "btn_up_right") {
        newBrightness = Math.min(newBrightness + STEP, 100);
      } else if (button === "btn_down_right") {
        newBrightness = Math.max(newBrightness - STEP, 0);
      }
    }

    // Apply brightness after all presses are processed
    if (button === "btn_up_right" && action === "long_press") {
      turnOn();
    } else if (button === "btn_down_right" && action === "long_press") {
      turnOff();
    } else if (button === "btn_up_right" || button === "btn_down_right") {
      setBrightness(newBrightness);
    }
  });
});
